var list, list_random, list_random_value;
list = ds_list_create()
ds_list_add(list, 0, 2, 3, 4, 5, 6, 7, 9, 10, 11, 12, 13, 14)
ds_list_shuffle(list)
list_random = ds_list_create()
ds_list_add(list_random, 0, 1, 2)
ds_list_shuffle(list_random)
list_random_value = ds_list_find_value(list_random, (ds_list_size(list_random) - 1))
if (list_random_value == 0)
    oControl.mod_bombs = 8
else if (oControl.mod_randomitems == 1)
{
    while 1
    {
        oControl.mod_bombs = ds_list_find_value(list, (ds_list_size(list) - 1))
        if (oControl.mod_bombs != 0)
            break
        else
        {
            ds_list_shuffle(list)
...
if (list_random_value == 1)
    oControl.mod_spider = 8
else if (oControl.mod_randomitems == 1)
{
    while 1
    {
        oControl.mod_spider = ds_list_find_value(list, (ds_list_size(list) - 1))
        if (oControl.mod_spider != 0)
            break
        else
        {
            ds_list_shuffle(list)
...
if (list_random_value == 2)
    oControl.mod_charge = 8
else if (oControl.mod_randomitems == 1)
{
    while 1
    {
        oControl.mod_charge = ds_list_find_value(list, (ds_list_size(list) - 1))
        if (oControl.mod_charge != 0)
            break
        else
        {
            ds_list_shuffle(list)
